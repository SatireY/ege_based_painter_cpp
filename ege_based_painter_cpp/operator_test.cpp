...
void operator_test()
{
    ...
    xyprintf(8, 8 + 7 * graphicsEnvironment.GetMenuHeight(),
             "A = Color{32, 64, 128}, A[0] = %d, A[1] = %d, A[2] = %d",
             Color{ 32, 64, 128 } [0],
             Color{ 32, 64, 128 } [1],
             Color{ 32, 64, 128 } [2]
            );
    ...
    {
        xyprintf(8, 8 + 8 * graphicsEnvironment.GetMenuHeight(),
                 "越界测试：A = Color{32, 64, 128}, A[0] = %s",
                 error.what());
    }
    ...
    Circle B = A;
    xyprintf(8, 8 + 21 * graphicsEnvironment.GetMenuHeight(),
             "将“半径为10坐标为(10,10)颜色为0xFFEECC不填充的圆” A 通过 “=” 拷贝为圆 B， A == B ? %s",
             A == B ? "true" : "false");
    xyprintf(8, 8 + 22 * graphicsEnvironment.GetMenuHeight(),
             "“半径为10坐标为(10,10)颜色为0xFFEECC不填充的圆” A, A[0] = %d, A[1] = %d",
             A[0],
             A[1]);
    ...
    {
        xyprintf(8, 8 + 23 * graphicsEnvironment.GetMenuHeight(),
                 "越界测试：A[3] = %s",
                 error.what());
    }

    setcolor(0x009000);
    xyprintf(8, 8 + 25 * graphicsEnvironment.GetMenuHeight(),
